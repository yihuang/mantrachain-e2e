...
@click.option("--fromimage", default="")
...
def run(outdir: str, datadir: str, binary, global_seq):
    ...
    try:
        return do_run(datadir, home, binary, group, global_seq, cfg)
    finally:
        # collect outputs
...
def do_run(datadir: Path, home: Path, cmd: str, group: str, global_seq: int, cfg: dict):
